...
• You can choose to record the **value change events** of specific variables by adding them to the **"Variable Monitor"** list in the **mapp Audit Configuration**  
    ...
    o The provided example uses variable **::AuditMgr:VariableMonitorExample**  

• The **localizable text definition** for these events is defined in the **TxtDatapoints.tmx** files  

    o Namespace **MpAudit/DP** for the exported text, and namespace **MpAudit/Display/DP** for the displayed text  

    o The **text ID** must be equal to the fully resolved variable name (**::AuditMgr:VariableMonitorExample** in this case)  
