...
* **mapp View content** to view the **audit list**
  * The audit systems use **separate display texts**
  * The text shown on the **HMI** is different from the text exported to the file
  * This allows a succinct message to be shown on the HMI, with optional timestamp, user, or other context
  * The exported audit file contains more detailed information directly in the message
...
* The **localizable text definition** for these events is defined in the **TxtDatapoints.tmx** files
  * Namespace **MpAudit/DP** for the exported text
  * Namespace **MpAudit/Display/DP** for the displayed text
  * The **text ID** must be equal to the fully resolved variable name  
    (**::AuditMgr:VariableMonitorExample** in this case)
